...
const CampaignModal = ({ onClose, onCampaignCreated }) => {
    ...
    const generateCampaignId = async () => {
        try {
            const response = await axios.get('https://skillmuni.in:8080/nextCampaignId');
    ...
    const downloadQRCode = () => {
        if (!data) {
            console.error("Campaign ID is missing. Please generate a campaign ID first.");
            return;
        }
    ...
    const handleCreateCampaign = async (e) => {
        ...
        if (!data || !campaignName || !scannerType || !description || !startDate || !endDate) {
        ...
        try {
            const response = await axios.post('https://skillmuni.in:8080/api/createNewCampaign', payload);
        ...
        <div>
            <div className="modal-overlay">
                ...
                <div className="modal-container">
                    ...
                    {alertMessage && (
                        <div className="alert alert-danger alert-message text-center" style={{ width: '100%', marginTop: '-20px', marginBottom: '15px' }}>
                            {" "}
                            {alertMessage}{" "}
                        </div>
                    )}
                    <section>
                        <form>
                            ...
                            <div className="form-group">
                                ...
                                <div className="form-floating">
                                    <input type="text" id="webURL" required placeholder="Web URL to access app (for QR code purpose)" className="form-control" value={qrscanData} onChange={(e) => setQrscanData(e.target.value)} />
                                    <label htmlFor="webURL">Web URL to access app (for QR code purpose)</label>
                                </div>
                            </div>
                            <div className="form-group">
                                <div className="form-floating">
                                ...
                                <div className="form-floating select-wrapper">
                                    <select
                                        ...
                                        value={scannerType} onChange={(e) => setScannerType(e.target.value)} onFocus={() => !scannerType && setScannerType("")}
                                    >
                                        <option value="" disabled hidden>Scanner Type</option>
                            ...
                            <div className="form-group">
                                ...
                                <div className='row'>
                                    <div className='col-md-6'>
                                        <button type="button" className='btn-generate-campaign' onClick={generateCampaignId}>Generate Campaign Id</button>
                                    </div>
                                    <div className='col-md-6'>
                                        <input type="text" id="campaignName" required className="form-control input-campaign" value={data} ref={qrRef} onChange={() => { }} disabled readOnly />
                                    </div>
                                </div>
                            </div>
                            {data &&
                                <div className="QR-form-group" style={{ marginLeft: "10%" }}>
                                    <button type="button" className='btn-generate-campaign btn-generate-QR' onClick={downloadQRCode} disabled={!generateQR || !data}>Download QR Code</button>
                                    <div ref={qrRef} className="input-QRCode">
                                        {generateQR && data && (
                                            (() => {
                                                return <QRCode value={`${qrscanData}${data}/${scannerType}`} size={100} level={"H"} style={{ marginTop: '-7px', marginLeft: '-9px' }} />;
                                            })()
                                        )}
                                    </div>
                                </div>
                            ...
                            <button type="button" className='btn-create-campaign' onClick={handleCreateCampaign}>Save</button>
                        </form>
                    </section>
                </div>
            </div>
