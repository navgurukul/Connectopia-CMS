  ...
  const generateCampaignId = async () => {
    try {
      const response = await axios.get(
        "https://15.206.198.172/nextCampaignId"
      );
  ...
  const handleCreateCampaign = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://15.206.198.172/api/createNewCampaign",
        payload
      );
      ...
      if (response.data) {
        ...
        try {
          const apiUrl = `https://15.206.198.172/updateimage/${data}/0/Main-QRCode/${scannerType}`;
