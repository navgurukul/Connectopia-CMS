...
export function UserGameReport({ setSelectedName, setSelectedDetail }) {
  ...
  useEffect(() => {
    ...
    if (userOrganization) {
      axios.get(`https://15.206.198.172/organisation/${userOrganization}`)
        .then((response) => {
  ...
  useEffect(() => {
    ...
    setSelectCampaignName("");
    if (organisationName) {
      axios.get(`https://15.206.198.172/organisation/${organisationName}`)
        .then((response) => {
          setCampaignList(response.data);
        })
        .catch((error) => {
  ...
  const fetchOrganisation = async () => {
    try {
      const apiUrl = `https://15.206.198.172/organisationlist/${userEmailid}/${userType}`;
  ...
  const fetchUsersByOrganisation = async (selectedOrganisation) => {
    try {
      const apiUrl = `https://15.206.198.172/users_by_organisation/${selectedOrganisation}`;
      const response = await fetch(apiUrl);
      const data = await response.json();
      setUserDetail(data);
    } catch (error) {
  ...
  const fetchUserReport = async (selectCampaignName) => {
    try {
      const apiUrl = `https://15.206.198.172/getPlayersList/${selectCampaignName}`;
      const response = await fetch(apiUrl);
      if (!response.ok) {
  ...
  const deleteUser = (phoneNumber) => {
    axios.delete(`https://15.206.198.172/deletePlayer/${phoneNumber}/${selectCampaignName}`)
      .then(response => {
        fetchUserReport();
      })
      .catch(error => {
